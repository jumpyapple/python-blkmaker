...
def init_generation3(tmpl, script, override_cb=False):
    if (not tmpl.cbtxn is None) and not (
        override_cb and ("generate" in tmpl.mutations)
    ):
        return (0, False)
...
def _build_merkle_branches(tmpl):
    ...
    while len(branches) < branchcount:
        ...
        merklehashes = [None] + [
            _dblsha256(merklehashes[i] + merklehashes[i + 1])
            for i in range(2, len(merklehashes), 2)
        ]
...
def _append_cb(tmpl, append, appended_at_offset=None):
    ...
    coinbase = (
        coinbase[:_cbScriptSigLen]
        + chr(newLen).encode("ascii")
        + coinbase[_cbScriptSigLen + 1 : cbExtraNonce]
        + append
        + coinbase[cbExtraNonce:]
    )
...
def append_coinbase_safe2(tmpl, append, extranoncesz=0, merkle_only=False):
    ...
    availsz = (
        coinbase_size_limit
        - extranoncesz
        - ord(tmpl.cbtxn.data[_cbScriptSigLen : _cbScriptSigLen + 1])
    )
...
def get_mdata(
    tmpl, usetime=None, out_expire=None, extranoncesz=sizeof_workid, can_roll_ntime=True
):
    if usetime is None:
        usetime = _time()
    if not (
...
def _submit(tmpl, data, extranonce, dataid, nonce, foreign):
    ...
    return {"id": 0, "method": "submitblock", "params": [blkhex, info]}
