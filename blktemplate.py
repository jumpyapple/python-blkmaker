...
class Template:
    ...
    def get_mdata(
        self, usetime=None, out_expire=None, extranoncesz=_blkmaker.sizeof_workid
    ):
    ...
    def add(self, json, time_rcvd=None):
        ...
        if self.version > _blkmaker.MAX_BLOCK_VERSION or (
            self.version >= 2 and not self.height
        ):
